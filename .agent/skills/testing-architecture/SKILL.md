...
2.  **Integration & E2E (Multi-module & Flows)**:
    - **Location**: All high-level and interaction tests are centralized in the `cypress/` directory at the project root.
    - **Structure**:
      - `cypress/integration/`: Interaction tests between multiple components or logic modules (using Vitest + happy-dom).
      - `cypress/e2e/`: Real browser end-to-end flows covering complete user journeys.

## 🧱 Test Tiers

| Tier | Scope | Framework | Execution Context |
| :--- | :--- | :--- | :--- |
| **Unit** | Atomic logic | Jasmine + Karma | Browser (ChromeHeadless) |
| **Integration** | Component/Service interactions | Vitest + ATL | Node (happy-dom) |
| **E2E** | User-facing flows | Cypress | Browser (Real) |
