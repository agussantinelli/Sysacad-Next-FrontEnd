...
@Component({
    ...
    template: `
    <app-page-layout title="Gestión de Facultades" icon="domain">
              ...
              <table class="data-table">
                ...
                <tbody>
                  @for (facultad of facultades; track facultad.id) {
                    ...
                    <tr>
                      <td colspan="5" class="empty-cell">No hay facultades registradas.</td>
                    </tr>
                  }
                </tbody>
              </table>
...
export class AdminFacultiesComponent implements OnInit {
    ...
    create() {
        ...
        this.adminService.createFacultad(this.newFacultad).subscribe({
            next: () => {
                this.alertService.success('Facultad creada correctamente');
                this.newFacultad = { ciudad: '', provincia: '' };
                this.loadFacultades();
            },
            error: (err) => {
                console.error(err);
                this.alertService.error('Error al crear facultad');
                this.isLoading = false;
            }
        });
    }
    ...
    delete(facultad: FacultadAdminDTO) {
        if (confirm(`¿Está seguro de eliminar la Facultad de ${facultad.ciudad}?`)) {
            ...
            this.adminService.deleteFacultad(facultad.id).subscribe({
                next: () => {
                    this.alertService.success('Facultad eliminada');
                    this.loadFacultades();
                },
                error: (err) => {
                    console.error(err);
                    this.alertService.error('Error al eliminar facultad');
                    this.isLoading = false;
                }
            });
        }
