...
describe('Admin Commissions Management Flow', () => {
  ...
  it('should create a commission and assign a subject with staff', () => {
    ...
    cy.url().should('include', '/admin/commissions');

    cy.log('2. Create a new dummy Commission');
    cy.get('button.btn-primary').contains('Nueva Comisión').click({ force: true });
    cy.get('input[placeholder="Ej: 1K1"]').type('E2E-TEST-99');
    ...
    cy.contains('E2E-TEST-99').click({ force: true });
    cy.get('button.btn-primary').contains('Agregar Materia').click({ force: true });
    ...
    cy.get('select#diaSelect').select('LUNES');
    ...
    cy.get('.professor-item').first().click({ force: true });
